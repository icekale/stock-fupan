...
def backtest_guarded_exit(
    rows: Sequence[dict[str, object]],
    *,
    top_n: int,
    guard_return_key: str,
    final_return_key: str,
    ...
    guard_exit_count = 0
    final_exit_count = 0
    guard_missing_count = 0
    for row in selected:
        guard_return = _float_or_none(row.get(guard_return_key))
        final_return = float(row.get(final_return_key) or 0.0)
        if guard_return is None:
            guard_missing_count += 1
            final_exit_count += 1
            returns.append(final_return - cost)
        elif guard_return < guard_threshold:
            guard_exit_count += 1
            returns.append(guard_return - cost)
        else:
            final_exit_count += 1
            returns.append(final_return - cost)
    ...
    return {
        "top_n": top_n,
        "guard_return_key": guard_return_key,
        "final_return_key": final_return_key,
        ...
        "guard_missing_count": guard_missing_count,
        "average_return": round(average_return, 6),
        "hit_3pct_rate": round(hit_3pct_rate, 6),
        "hit_5pct_rate": round(hit_5pct_rate, 6),
        "win_rate": round(win_rate, 6),
