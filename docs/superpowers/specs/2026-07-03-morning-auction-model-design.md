...
```text
竞价数据：9:15-9:25 快照，至少需要 9:25 开盘价、竞价量、竞价额、委买委卖、未匹配量
日 K 数据：前复权开高低收、成交量、成交额、换手率，至少 120-250 个交易日
资金数据：近 3 日/5 日主力净流入、大单净量，能拿到就用，拿不到不阻塞 V1
板块数据：所属行业/概念、板块涨幅、板块涨停数、板块强度排名
特殊数据：龙虎榜、昨日涨停/炸板/连板信息，作为增强因子
```
